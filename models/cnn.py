...
class M5(nn.Module):
    ...
    def __init__(self, n_input: int = 1, n_output: int = 35, 
        ...
        self.conv4 = nn.Conv2d(n_channel, n_channel, kernel_size=(1, 3))
        self.bn4 = nn.BatchNorm2d(n_channel, momentum=0.1)
...
if __name__ == "__main__":
    stride =8
    model = M5(length=2714, stride=stride)
    x = torch.randn(1, 1, 2714)
    print(model(x).shape)
    
    model = M5(length=4000, stride=stride)
