...
def _process_lightness_single_spec(args, model, spec_dir, lightness_type, w, x, y, logit, count, seed):
    max_delta_dict = {
        'spotlight': 3.0,
        'gradient_x': 3.0,
    }
    try:
        base_name = f"{count}_{seed}_{args.task}_{args.model}_{lightness_type}_{w}"
        ...
        perturbed_layer = LightnessPerturbationLayer(
            image=x.squeeze(0), 
        ...
        return {'success': True, 'result': res, 'task': (args, model, spec_dir, lightness_type, w, x, y, logit, count, seed)}
    except Exception as e:
        import traceback; traceback.print_exc()
        print(f"Failed to process {count}th spec")
        return {'success': False, 'task': (args, model, spec_dir, lightness_type, w, x, y, logit, count, seed)}
            
             
def generate_lightness_spec(args, model, test_loader, label_to_index, device):

    print(f'\n{"="*80}')
    print(f"Starting {args.task} {args.model} Lightness specs generation...")
    print(f'{"="*80}\n')
    
    valid_data = get_valid_data(args, model, test_loader, label_to_index, device)
    
    spec_dir = os.path.join(args.spec_dir, args.task, f'lightness_{args.model}')
    ...
    for lightness_type in ['spotlight', 'gradient_x']:
        for w in [0.2, 0.4, 0.5, 0.6, 0.8, 1.0]:
            for x, y, logit in valid_data:
                tasks.append((args, model, spec_dir, lightness_type, w, x, y, logit, count, args.seed))
                count += 1
    
    results = Parallel(n_jobs=os.cpu_count() // 2)(
        delayed(_process_lightness_single_spec)(*task) for task in tqdm(tasks)
    )
    # results = []
    ...
    for failed_result in tqdm(failed_results):
        retry_result = _process_lightness_single_spec(*failed_result['task'])
    ...
    print(f"[!] Lightness: {len(successful_results)} specs")
    gc.collect()
